...
uint16_t readLDR(uint16_t value){
	HAL_ADC_Start(&hadc);
...
uint16_t temperatureToPWM(uint16_t temperature) {
...
int main(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_SET); // PC9 -> IN1
  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_8, GPIO_PIN_RESET); // PC8 -> IN2
